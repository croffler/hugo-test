...
|`updateMode` | Push or pull update mode. Example: <br>`jini://localhost:10098/containerName /mySpaces?useLocalCache&updateMode=1` | `UPDATE_`{{% wbr %}} `MODE`{{% wbr %}} `_PULL`{{% wbr %}} `= 1` {{% wbr %}} `UPDATE_`{{% wbr %}} `MODE`{{% wbr %}} `_PUSH`{{% wbr %}} `= 2` |
